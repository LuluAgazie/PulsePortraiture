...
class GetTOAs(object):
    ...
    def __init__(self, datafiles, modelfile, quiet=False):
        ...
        if file_is_type(datafiles, "ASCII"):
            self.datafiles = [
                datafile[:-1] for datafile in open(datafiles, "r").readlines()
            ]
        else:
            self.datafiles = [datafiles]
        if len(self.datafiles) > max_nfile:
            print(
                "Too many archives.  See/change max_nfile(=%d) in pptoas.py."
                % max_nfile
            )
            sys.exit()
        ...
        self.summed_red_chi2s = []  # summed per-channel reduced chi2 values
        self.summed_chi2s = []  # summed per-channel chi2 values
        self.channel_chi2s = []  # per-channel chi2 values
    ...
    def get_TOAs(
        ...
        warning_message = "You are using an experimental functionality of pptoas!"
        self.nfit = 1
        if fit_DM:
            self.nfit += 1
        if fit_GM:
        ...
        for iarch, datafile in enumerate(datafiles):
            ...
            try:
                ...
                if len(data.SNRs[np.isnan(data.SNRs)]) > 0:
                    print(
                        "This file has %s frequency channels with a nan SNR"
                        % len(data.SNRs[np.isnan(data.SNRs)])
                    )
                    ...
                    for isub in data.ok_isubs:
                        for ipol in range(data.npol):
                            ...
                            data.ok_ichans[isub] = data.ok_ichans[isub][
                                ~np.isnan(
                                    data.SNRs[isub, ipol][
                                        np.array(data.ok_ichans[isub])
                                    ]
                                )
                            ]
                            # print(data.ok_ichans[isub])
                if not len(data.ok_isubs):
                    if not quiet:
                        print("No subints to fit for %s.  Skipping it." % datafile)
                    continue
                else:
            ...
            # Observation info
            obs = DataBunch(
                telescope=d.telescope, backend=d.backend, frontend=d.frontend
            )
            ...
            covariances = np.zeros([nsub, self.nfit, self.nfit], dtype=np.float64)
            nfevals = np.zeros(nsub, dtype="int")
            rcs = np.zeros(nsub, dtype="int")
            # PSRCHIVE epochs are *midpoint* of the integration
            MJDs = np.array(
                [d.epochs[isub].in_days() for isub in range(nsub)], dtype=np.double
            )
            ...
            if self.is_FITS_model:
                ...
                if model_data.nbin != nbin:
                    print(
                        "Model nbin %d != data nbin %d for %s; skipping it."
                ...
                if model_data.nchan != nchan:
                    print(
                        "Model nchan %d != data nchan %d for %s; skipping it."
                        % (model_data.nchan, nchan, datafile)
                    )
                    continue
            ...
            for isub in d.ok_isubs:
                if self.is_FITS_model and np.any(
                    model_data.freqs[0] - d.freqs[isub]
                ):  # tscrunched
                ...
                if not self.is_FITS_model:
                    ...
                    try:
                        if not fit_scat:
                            ...
                            self.model_name, self.ngauss, full_model = read_model(
                                self.modelfile,
                                d.phases,
                                d.freqs[isub],
                                d.Ps[isub],
                            ...
                            (
                                ...
                                self.modelfile, quiet=bool(quiet + (itoa - 1))
                            )
                        ...
                        self.model_name, model = read_spline_model(
                            self.modelfile, d.freqs[isub], nbin, quiet=True
                        )  # bool(quiet+(itoa-1)))
                # else:
                ...
                if add_instrumental_response and (
                    ...
                    modelx = fft.irfft(
                        inst_resp_port_FT * fft.rfft(modelx, axis=-1), axis=-1
                    )
                ...
                ###################
                DM_guess = DM_stored
                rot_port = rotate_data(
                    portx, 0.0, DM_guess, P, freqsx, nu_mean
                )  # why not nu_fit?
                ...
                if fit_scat:
                    if self.scat_guess is not None:
                        ...
                        tau_guess = (old_div(tau_guess_s, P)) * (
                            old_div(nu_fit_tau, tau_guess_ref)
                        ) ** alpha_guess
                    else:
                        ...
                        if hasattr(self, "gparams"):
                            tau_guess = (old_div(self.gparams[1], P)) * (
                                old_div(nu_fit_tau, self.model_nu_ref)
                            ) ** alpha_guess
                        else:
                            tau_guess = 0.0  # nbin**-1?
                            # tau_guess = guess_tau(...)
                    model_prof_scat = fft.irfft(
                        scattering_portrait_FT(
                            np.array(
                                [
                                    scattering_times(
                                        tau_guess, alpha_guess, nu_fit_tau, nu_fit_tau
                                    )
                                ]
                            ),
                            nbin,
                        )[0]
                        * fft.rfft(modelx.mean(axis=0))
                    )
                    phi_guess = fit_phase_shift(rot_prof, model_prof_scat, Ns=100).phase
                    if self.log10_tau:
                        if tau_guess == 0.0:
                            tau_guess = nbin**-1
                    ...
                    phi_guess = fit_phase_shift(
                        rot_prof, modelx.mean(axis=0), Ns=100
                    ).phase
                phi_guess = phase_transform(
                    phi_guess, DM_guess, nu_mean, nu_fit_DM, P, mod=True
                )  # why not use nu_fit at first?
                # Need a status bar?
                param_guesses = [phi_guess, DM_guess, GM_guess, tau_guess, alpha_guess]
                ...
                if len(freqsx) == 1:
                    ...
                    if not quiet:
                        print(
                            "TOA #%d only has 1 frequency channel...fitting for phase only..."
                            % (itoa)
                        )
                    ...
                    if not quiet:
                        print(
                            "TOA #%d only has 2 frequency channels...fitting for phase and DM only..."
                            % (itoa)
                        )
                else:
                    fit_flags = list(np.copy(self.fit_flags))
                results = fit_portrait_full(
                    portx,
                    modelx,
                    param_guesses,
                    P,
                ...
                results.TOA = epoch + pr.MJD(
                    old_div(((results.phi * P) + d.backend_delay), (3600 * 24.0))
                )
                ...
                if print_flux:
                    if results.tau != 0.0:
                        if self.log10_tau:
                            ...
                            tau = results.tau
                        alpha = results.alpha
                        scat_model = fft.irfft(
                            scattering_portrait_FT(
                                scattering_times(tau, alpha, freqsx, results.nu_tau),
                                data.nbin,
                            )
                            * fft.rfft(modelx, axis=1),
                            axis=1,
                        )
                    else:
                        scat_model = np.copy(modelx)
                    scat_model_means = scat_model.mean(axis=1)
                    profile_fluxes[isub, d.ok_ichans[isub]] = (
                        scat_model_means * results.scales
                    )
                    profile_flux_errs[isub, d.ok_ichans[isub]] = (
                        abs(scat_model_means) * results.scale_errs
                    )
                    flux, flux_err = weighted_mean(
                        profile_fluxes[isub, d.ok_ichans[isub]],
                        profile_flux_errs[isub, d.ok_ichans[isub]],
                    )
                    flux_freq, flux_freq_err = weighted_mean(
                        freqsx, profile_flux_errs[isub, d.ok_ichans[isub]]
                    )
                    fluxes[isub] = flux
                ...
                try:
                    ...
                    for ii, ifit in enumerate(np.where(fit_flags)[0]):
                        for jj, jfit in enumerate(np.where(fit_flags)[0]):
                            covariances[isub][ifit, jfit] = results.covariance_matrix[
                                ii, jj
                            ]
                ...
                if fit_flags[3]:
                    if self.log10_tau:
                        ...
                        # usec, w/ df
                        toa_flags["log10_scat_time"] = results.tau + np.log10(
                            old_div(P, df)
                        )  # w/ df
                        ...
                        toa_flags["scat_time_err"] = (
                            old_div(results.tau_err * P, df) * 1e6
                        )  # usec, w/ df
            ...
            DeltaDM_mean, DeltaDM_var = np.average(
                DeltaDMs[d.ok_isubs], weights=DM_weights, returned=True
            )
            ...
            if len(d.ok_isubs) > 1:
                ...
                # errors.
                DeltaDM_var *= old_div(
                    np.sum(((DeltaDMs[d.ok_isubs] - DeltaDM_mean) ** 2) * DM_weights),
                    (len(DeltaDMs[d.ok_isubs]) - 1),
                )
            ...
            if not quiet:
                ...
                print(
                    "Med. TOA error is %.3f us"
                    % (np.median(phi_errs[d.ok_isubs]) * d.Ps.mean() * 1e6)
                )
            if show_plot:
                stop = time.time()
            ...
            if not quiet and len(self.ok_isubs):
                print("--------------------------")
                print(
                    "Total time: %.2f sec, ~%.4f sec/TOA"
                    % (
                        tot_duration,
                        old_div(
                            tot_duration,
                            (np.array(list(map(len, self.ok_isubs))).sum()),
                        ),
                    )
                )

    def get_narrowband_TOAs(
        self,
        datafile=None,
        tscrunch=False,
        fit_scat=False,
        ...
        for iarch, datafile in enumerate(datafiles):
            ...
            covariances = np.zeros(
                [nsub, nchan, self.nfit, self.nfit], dtype=np.float64
            )
            ...
            for isub in d.ok_isubs:
                    ...
                    try:
                        if not fit_scat:
                            ...
                            (
                                ...
                                self.modelfile, quiet=bool(quiet + (icount - 1))
                            )
                            unscat_params = np.copy(self.gparams)
                            unscat_params[1] = 0.0
                            model = unscat_model = gen_gaussian_portrait(
                ...
                for ichanx in range(len(d.ok_ichans[isub])):
                    ...
                    if self.is_FITS_model:
                        if model_data.weights[isub, ichan] == 0:
                            continue
                    prof = portx[ichanx]
                    ...
                    results = fit_phase_shift(
                        prof, model_prof, err, bounds=[-0.5, 0.5], Ns=100
                    )
                    ...
                    if print_flux:
                        if results.tau != 0.0:
                            ...
                            alpha = results.alpha
                            scat_model = fft.irfft(
                                scattering_profile_FT(tau, data.nbin)
                                * fft.rfft(model_prof)
                            )
                        else:
                            scat_model = np.copy(model_prof)
                        scat_model_mean = scat_model.mean()
                        profile_fluxes[isub, ichan] = scat_model_mean * results.scale
                    ...
                    if fit_flags[1]:
                        ...
                        toa_flags["phi_tau_cov"] = results.covariance_matrix[0, 1]
                    toa_flags["be"] = d.backend
                    toa_flags["fe"] = d.frontend
                    toa_flags["f"] = d.frontend + "_" + d.backend
                    toa_flags["nbin"] = nbin
        ...
        if not quiet and len(self.ok_isubs):
            ...
            print(
                "Total time: %.2f sec, ~%.4f sec/TOA"
                % (tot_duration, tot_duration / len(self.TOA_list))
            )

    def get_psrchive_TOAs(
        self,
        datafile=None,
        tscrunch=False,
        algorithm="PGS",
        ...
        for iarch, datafile in enumerate(datafiles):
            ...
            if not self.is_FITS_model:
                ...
                freqs = np.array(
                    [
                        [sub.get_centre_frequency(ichan) for ichan in range(nchan)]
                        for sub in arch
                    ]
                )
                for isub in range(nsub):
                    ...
                    try:
                        Ps = np.array(
                            [sub.get_folding_period() for sub in arch], dtype=np.double
                        )
                        phases = get_bin_centers(nbin, lo=0.0, hi=1.0)
                        self.model_name, self.ngauss, model = read_model(
                            self.modelfile, phases, freqs[isub], Ps[isub], quiet=True
                        )
                    except (UnboundLocalError, UnicodeDecodeError):
                        self.model_name, model = read_spline_model(
    ...
    def get_channels_to_zap(
        self, SNR_threshold=8.0, rchi2_threshold=1.3, iterate=True, show=False
    ):
        """
        ...
        for iarch, ok_idatafile in enumerate(self.ok_idatafiles):
            ...
            for isub in self.ok_isubs[iarch]:
                ...
                channel_SNR_threshold = (
                    old_div(SNR_threshold**2.0, len(ok_ichans))
                ) ** 0.5
                for ichan, ok_ichan in enumerate(ok_ichans):
                    channel_red_chi2 = get_red_chi2(
                        port[ok_ichan],
                        model[ok_ichan],
                        errs=noise_stds[ok_ichan],
                        dof=len(port[ok_ichan]) - 2,
                    )  # Not sure about dof
                    channel_chi2 = channel_red_chi2 * (len(port[ok_ichan]) - 2.0)
                    chi2s.append(channel_chi2)
                    ...
                    if channel_red_chi2 > rchi2_threshold:
                        ...
                        SNR_threshold and channel_snrs[ok_ichan] < channel_SNR_threshold
                    ):
                        bad_ichans.append(ok_ichan)
                    else:
                        pass
                chi2s = np.array(chi2s)
                summed_chi2s.append(np.sum(chi2s))
                summed_red_chi2s.append(
                    np.sum(chi2s) / ((len(port[0]) - 2.0) * len(ok_ichans))
                )
                channel_chi2s.append(chi2s)
                ...
                if iterate and SNR_threshold and len(bad_ichans):
                    ...
                    while added_new and (len(ok_ichans) - len(bad_ichans)):
                        ...
                        channel_SNR_threshold = (
                            old_div(
                                SNR_threshold**2.0, (len(ok_ichans) - len(bad_ichans))
                            )
                        ) ** 0.5
                        for ichan, ok_ichan in enumerate(ok_ichans):
                ...
                if show and len(bad_ichans):
                    show_portrait(
                        port,
                        get_bin_centers(port.shape[1]),
                        title="%s, subint: %d\nbad chans: %s"
                        % (datafile, isub, bad_ichans),
                        show=False,
                    )
                    port[bad_ichans] *= 0.0
                    show_portrait(
            ...
            self.summed_red_chi2s.append((summed_red_chi2s))
            self.summed_chi2s.append((summed_chi2s))
            self.channel_chi2s.append((channel_chi2s))
            self.channel_red_chi2s.append((channel_red_chi2s))
    ...
    def show_fit(
        ...
        if self.is_FITS_model:
            ...
            model = (model_data.masks * model_data.subints)[0, 0]
            if model_data.nchan == 1:
                model = np.tile(model[0], len(freqs)).reshape(
                    len(freqs), model_data.nbin
                )
            model_name = self.modelfile
        else:
            try:
                model_name, ngauss, model = read_model(
                    self.modelfile, phases, freqs, data.Ps.mean(), quiet=quiet
                )
                ...
                if self.taus[ifile][isub] != 0.0:
                    ...
                    gparams[1] = 0.0
                    model = gen_gaussian_portrait(
                        model_code, gparams, 0.0, phases, freqs, model_nu_ref
                    )
            except:
                model_name, model = read_spline_model(
                    self.modelfile, freqs, data.nbin, quiet=True
                )
        if self.add_instrumental_response and (self.ird["DM"] or len(self.ird["wids"])):
            inst_resp_port_FT = instrumental_response_port_FT(
                data.nbin,
                freqs,
                self.ird["DM"],
            ...
            model = fft.irfft(inst_resp_port_FT * fft.rfft(model), axis=-1)
        if self.taus[ifile][isub] != 0.0:
            ...
            model = fft.irfft(
                ...
                * fft.rfft(model, axis=1),
                axis=1,
            )
        port = rotate_portrait_full(
            data.subints[isub, 0], phi, DM, GM, freqs, nu_ref_DM, nu_ref_GM, P
        )
        if rotate:
        ...
        if show:
            show_residual_plot(
                ...
                phases=phases,
                freqs=freqs,
                noise_stds=data.noise_stds[isub, 0],
                nfit=2,
                titles=titles,
                rvrsd=bool(data.bw < 0),
                savefig=savefig,
...
if __name__ == "__main__":
    ...
    if not narrowband and not psrchive:  # get wideband TOAs
        ...
        gt.get_psrchive_TOAs(
            ...
            flags="IPTA",
            attributes=["chan", "subint"],
        )  # , print_toas=True, outfile=None)
    if not psrchive:
        if format == "princeton":
            gt.write_princeton_TOAs(outfile=outfile, one_DM=one_DM, dmerrfile=errfile)
        else:
            if one_DM:
